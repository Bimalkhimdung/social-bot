        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {article?.scraped_at && (
            <div style={{ fontSize: '0.7rem', color: 'var(--text-muted)', marginTop: 6, display: 'flex', alignItems: 'center', gap: 4 }}>
              ...
              <span>{new Date(article.scraped_at).toLocaleString('en-US', { 
                timeZone: 'Asia/Kathmandu', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit', hour12: false 
              })}</span>
            </div>
          )}
          ...
          {posted_at && (
            <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginTop: 4 }}>
              Posted: {new Date(posted_at).toLocaleString('en-US', { timeZone: 'Asia/Kathmandu' })}
            </div>
          )}
          {scheduled_at && !posted_at && (
            <div style={{ fontSize: '0.75rem', color: 'var(--amber)', marginTop: 4 }}>
              Scheduled: {new Date(scheduled_at).toLocaleString('en-US', { timeZone: 'Asia/Kathmandu' })}
            </div>
          )}
        </div>
      </div>
